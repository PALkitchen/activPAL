...
## functions to identify possible exercise -----------------------------
...
parse.device.serial <-  function(file_name) {
...
sed.activity.marker <-
  function(events_file, time_value, duration) {
    ...
    for (i in (1:te)) {
      sed.temp.events <- events_file[which(events_file$Time >= time_value$Time[i] &
...
transportation.activity.marker <-
  function(events_file, time_value, duration) {
    ...
    for (i in (1:te)) {
      transportation.temp.events <- events_file[which(events_file$Time >= time_value$Time[i] &
                                                        events_file$Time <= time_value$end_time[i] &
                                                        events_file$Event.Type == 5 &
                                                        events_file$Duration..s. >= duration), ]
...
exercise.log <-
  function(time_value, folder_location, id) {
    ...
    exercise.log <- read.csv(paste(folder_location,
                                   "exercise_log.csv",
                                   sep = ""),
                             row.names = NULL, sep = ",",
                             stringsAsFactors = FALSE)
    ...
    if (tl > 0) {
      temp.log$date <- as.POSIXct(temp.log$date,
                                  format = "%m/%d/%y",
                                  tz = "UTC")
...
make.index.file <- function(Events_Files_To_Process_folder_location,
                            Temp_Output_folder_location) {
...
activpal.stepping.test.day <-
  function(file.data, window.size) {
    library(Rcpp)
    file.data$seconds <- round(as.numeric(difftime(file.data$Time,
                                                   file.data$date,
                                                   units = "secs")), 1)
